...
public class MessageText implements IGeneric
{
    ...
    {
        Messages.Quote.Builder quote = Messages.Quote.newBuilder().setQuotedMessageId(msgId.toString()).setQuotedMessageSha256(ByteString.copyFrom(sha256));
    ...
    {
        Messages.Mention.Builder mention = Messages.Mention.newBuilder().setUserId(mentionUser.toString()).setLength(len).setStart(offset);
